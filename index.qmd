...
func_tab_esc_dois_ou_mais_anos <- function(ano, base_dados_censo_esc) {
  ...
  base_dados_censo_esc |>
    # Selecione as escolas que aparecem em mais de um ano
    filter(no_censos_pres > 1 & ano_cn_escolar == ano) |>

    # Agrupe as escolas em função dos anos em que elas se encontram presentes
    group_by(tipo_censos_pres) |>

    # Compute o número de escolas de acordo com os anos em que elas se encontram presentes
    count(tipo_censos_pres, ano_censos_pres, name = "tot") |>
    ungroup()
}

# Produza a lista de tabelas (data.frames puros) por ano
list_tab <- map(
  censo_esc_anos,
...
# Apresente as tabelas
for (i in seq_along(list_tab)) {
  
  # Captura o ano corrente
  ano_atual <- censo_esc_anos[i]
  
  # Cria o título exclusivo da tabela
  titulo_tabela <- str_c("Dados do Censo Escolar referentes ao ano de ", ano_atual)
  
  # Cria a tabela usando o formato clássico de Markdown do kable
  tabela_gerada <- knitr::kable(
    list_tab[[i]], 
    # format = "simple", # O formato 'simple' ativa os estilos nativos e limpos do Quarto
    col.names = c("Presença em mais de um censo escolar", "Anos", "Total de escolas"), 
    align = "c",
    caption = titulo_tabela
  )
  
  # Envia o Markdown estruturado direto para o interpretador visual do Quarto
  # cat(knitr::asis_output(tabela_gerada))
  print(tabela_gerada)
  
  # Adiciona espaçamentos obrigatórios para empilhar verticalmente sem fundir as tabelas
  cat("\n\n<br>\n\n")
}
